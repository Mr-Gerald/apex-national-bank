...
const VerifyProfileLinkCardScreen: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        ...
        const linkedCard = await linkExternalCard(newCardData); 
        ...
        setTimeout(() => {
            setIsLoading(false);
            navigate(`/profile-verification/pin/${linkedCard.id}`);
        }, 1200);
    } catch (error: any) {
  ...
  return (
    <div className="p-4 space-y-6">
      <StepIndicator currentStep={3} totalSteps={4} flowType="profile" />
      <h1 className="text-2xl font-semibold text-neutral-800">Verify for Top-Tier Access (Step 3/4)</h1>
      <p className="text-sm text-neutral-600">Link an external debit or credit card as part of the identity verification process. This card will be saved to your profile.</p>
      ...
      <form onSubmit={handleSubmit} className="bg-white p-6 rounded-lg shadow-md space-y-4">
        ...
        <div className="flex space-x-3">
            <Button type="button" variant="outline" onClick={() => navigate('/profile-verification/upload-id')} disabled={isLoading}>
                Back
            </Button>
      ...
      <div className="text-xs text-neutral-500 p-3 bg-neutral-100 rounded-md border border-neutral-200 flex items-start space-x-2">
        ...
        <span>Your card information is handled securely. This step helps us confirm your identity.</span>
      </div>
    </div>
  );
};

export default VerifyProfileLinkCardScreen;
