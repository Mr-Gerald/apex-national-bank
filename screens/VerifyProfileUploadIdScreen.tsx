...
const VerifyProfileUploadIdScreen: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await saveVerificationIdImages(frontIdImage, backIdImage, true); // true for profile verification
      // Simulate processing delay
      setTimeout(() => {
        setIsLoading(false);
        navigate(`/profile-verification/link-card`);
      }, 1200);
    } catch (error: any) {
  ...
  return (
    <div className="p-4 space-y-6">
      <StepIndicator currentStep={2} totalSteps={4} flowType="profile" />
      <h1 className="text-2xl font-semibold text-neutral-800">Verify for Top-Tier Access (Step 2/4)</h1>
      ...
      <form onSubmit={handleSubmit} className="bg-white p-6 rounded-lg shadow-md space-y-4">
        ...
        <div className="flex space-x-3">
            <Button type="button" variant="outline" onClick={() => navigate('/profile-verification/data')} disabled={isLoading}>
                Back
            </Button>
...
export default VerifyProfileUploadIdScreen;
