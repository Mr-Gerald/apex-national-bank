...
const ACHTransferScreen: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setIsLoading(false);
      setMessage(`Successfully initiated ${transferType.toUpperCase()} transfer of ${formatCurrency(numericAmount)} to ${recipientName}. Transaction ID: ACH${Date.now().toString().slice(-6)}. Please allow 1-3 business days for ACH or up to 1 business day for Wire transfers to complete.`);
      setRecipientName('');
      setRoutingNumber('');
      setAccountNumber('');
      setAmount('');
